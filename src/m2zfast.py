...
def read_metal(metal_file,snp_column,pval_column,no_transform,chr,start,end,db_file,delim):
  ...
  for line in f:
    ...
    snp = e[snp_col]
    # CHANGE WARNING NOTICE CODE
    # THIS IS WHERE I MADE A CHANGE! I COMMENTED THIS OUT!
...
def runAll(input_file,input_type,refsnp,chr,start,end,opts,args):
  ...
  if not opts.no_ld:
    ...
    if opts.ld:
      ...
      #CHANGE IN CODE WARNING
      #ADDED chr IN THE LINE BELOW TO FIT WITH GRCH38
      tabix_region = "chr{0}:{1}-{2}".format(chr,start,end)
